...
public class DSA extends MIDlet implements CommandListener {
  ...
  public void startApp() {
    Form form = new Form("DSA example");
  ...
  public void commandAction(Command command, Displayable screen) {
    if (command == exitCommand) {
      ...
      try {
        ...
        engine.generateDSAKeyPair ();
        endTime = (new Date ()).getTime();
        interval = endTime - startTime;

        Alert a = new Alert("success!");
        a.setString("DSA Key is generated in " 
                    + interval + " millisec");
      ...
      try {
        ...
        startTime = (new Date ()).getTime();
        signature = engine.DSASign(message);
        endTime = (new Date ()).getTime();
        interval = endTime - startTime;
 
        Form form = new Form ("DSA Signed");
        form.append ("DSA sigining finished in " 
                     + interval + " millisec\n");
      ...
      try {
        startTime = (new Date ()).getTime();
        boolean valid = engine.DSAVerify(message, signature);
        endTime = (new Date ()).getTime();
        interval = endTime - startTime;
 
        Form form = new Form ("DSA signatured verified");
        form.append ("DSA signature verification finished in " 
                     + interval + " millisec\n");
        form.append ("Result:\n");
        if ( valid ) {
          form.append("success!");
