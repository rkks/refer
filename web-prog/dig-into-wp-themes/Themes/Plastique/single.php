					...
					<div<?php if ($nu_columns == 'Three Columns') { echo ' class="posts_three-columns"'; } elseif ($nu_columns == 'Single Column') { echo ' class="posts_single-column"'; } elseif ($nu_columns == 'Left Sidebar') { echo ' class="posts_left-sidebar"'; } elseif ($nu_columns == 'Right Sidebar') { echo ' class="posts_right-sidebar"'; } ?> id="posts">
						...
						<div <?php post_class(); ?> id="post-<?php the_ID(); ?>">
							...
							<div class="post-wrap-2">
								...
								<div class="entry">
									<?php the_content(''); ?>
									<?php wp_link_pages(array('before'=>'<p class="paged-links"><strong>Pages:</strong> ', 'after'=>'</p>', 'next_or_number'=>'number')); ?>
								</div>
								<div class="post-metadata">
									<div class="post-metadata-info">
										<div class="post-metadata-caption">Metadata</div>
										<ul>
											<li>Categorized as <?php the_category(', '); ?></li>

											<?php if (('open' == $post->comment_status) && ('open' == $post->ping_status)) { // both comments and pings open ?>
											<li><a href="#respond">Comment</a> or <a href="<?php trackback_url(); ?>" rel="trackback">trackback</a> for this post.</li>

											<?php } elseif (!('open' == $post->comment_status) && ('open' == $post->ping_status)) { // comments closed, pings open ?>
											<li>Comments are closed, but you may <a href="<?php trackback_url(); ?>" rel="trackback">trackback</a> this post.</li>

											<?php } elseif (('open' == $post-> comment_status) && !('open' == $post->ping_status)) { // comments open, pings closed ?>
											<li>Pings are closed. Leave a <a href="#respond">comment</a>.</li>

											<?php } elseif (!('open' == $post-> comment_status) && !('open' == $post->ping_status)) { // comments and pings closed ?>
											<li>Both comments and pings are closed.</li>
											<? } ?>
									...
									<?php if ($nu_after_post == 'true') { ?>

									<div class="post-metadata-social">
										<?php if (function_exists('dynamic_sidebar') && dynamic_sidebar('after-post')) : else : ?>
										<div class="pre-widget">
											<p><strong>After Post</strong></p>
											<p>This panel is active and ready for you to add some widgets via the WP Admin.</p>
										</div>
										<?php endif; ?>
									</div>
								...
								<?php //edit_post_link('Edit this entry','<p class="edit-post">','</p>'); ?>
							</div>
							</div>
						</div>
