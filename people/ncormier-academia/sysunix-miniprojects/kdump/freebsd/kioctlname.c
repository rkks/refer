...
ioctlname(u_long val)
{
	...
	if (val ==  DIOCRINACOMMIT)
		return("DIOCRINACOMMIT");
	if (val ==  METEORGCONT)
		return("METEORGCONT");
	if (val ==  FIOGETLBA)
		return("FIOGETLBA");
	...
	if (val ==  ACPIIO_BATT_GET_TYPE)
		return("ACPIIO_BATT_GET_TYPE");
	...
	if (val ==  DIOCGETANCHORS)
		return("DIOCGETANCHORS");
	...
	if (val ==  CDRIOCFIXATE)
		return("CDRIOCFIXATE");
	if (val ==  DIOCBEGINALTQS)
		return("DIOCBEGINALTQS");
	if (val ==  CONS_SCRSHOT)
		return("CONS_SCRSHOT");
	if (val ==  CONS_IO)
		return("CONS_IO");
	...
	if (val ==  ACPIIO_CMBAT_GET_BST)
		return("ACPIIO_CMBAT_GET_BST");
	if (val ==  SIOCADDRT)
		return("SIOCADDRT");
	if (val ==  METEORSTS)
		return("METEORSTS");
	...
	if (val ==  ACPIIO_CMBAT_GET_BIF)
		return("ACPIIO_CMBAT_GET_BIF");
	if (val ==  TIOCSETA)
		return("TIOCSETA");
	if (val ==  TIOCSETC)
		return("TIOCSETC");
	...
	if (val ==  DIOCBEGINRULES)
		return("DIOCBEGINRULES");
	...
	if (val ==  DIOCGETANCHOR)
		return("DIOCGETANCHOR");
	if (val ==  SMBIOC_READ)
		return("SMBIOC_READ");
	if (val ==  DIOCSTOPALTQ)
		return("DIOCSTOPALTQ");
	...
	if (val ==  DIOCADDSTATE)
		return("DIOCADDSTATE");
	if (val ==  DIOCRINABEGIN)
		return("DIOCRINABEGIN");
	...
	if (val ==  DIOCCOMMITRULES)
		return("DIOCCOMMITRULES");
	...
	if (val ==  ACPIIO_BATT_GET_BATTDESC)
		return("ACPIIO_BATT_GET_BATTDESC");
	if (val ==  PIO_FONT8x8)
		return("PIO_FONT8x8");
	if (val ==  TIOCLBIC)
		return("TIOCLBIC");
	...
	if (val ==  MDIOCLIST)
		return("MDIOCLIST");
	if (val ==  DIOCCOMMITALTQS)
		return("DIOCCOMMITALTQS");
	if (val ==  KBDGCKEY)
		return("KBDGCKEY");
