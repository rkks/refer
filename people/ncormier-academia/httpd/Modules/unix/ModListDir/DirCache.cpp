  ...
  namespace listdir
  {
    ...
    void  DirCache::Add(FileInfo* elem)
    {
      if (strcmp(elem->File, ".") == 0)
        this->_cachedMDate = elem->Mdate;
      if (this->_maxSize < elem->GetSize().length())
        this->_maxSize = elem->GetSize().length() + 1;
      if (this->_maxFile < strlen(elem->File))
    ...
    void DirCache::SortByName()
    {
      ...
      for (list<FileInfo*>::iterator ite = this->Files.begin();
	   ite != this->Files.end(); ite++)
        for (list<FileInfo*>::iterator ptr = ite; ptr != this->Files.end();
	     ptr++)
