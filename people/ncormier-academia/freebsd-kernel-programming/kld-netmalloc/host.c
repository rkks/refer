...
static void	host_query_allocation(host_t* self, unsigned size);
...
host_t*	alloc_host(void)
{
  ...
  host->query_allocation = host_query_allocation;
...
static void	host_recv(host_t* self)
{
  ...
  switch (buf[0])
	...
	dist_elem_t* dist_elem = get_server()->get_dist_elem_by_id(get_server(), id);
	if (!dist_elem) die("host_recv: can't find dist_elem");
	elem_t* elem = dist_elem->local_elem;
	if (elem)
    ...
    case CREATE_REQUEST:
      {
	unsigned size = (unsigned) buf[2];
	int id = get_server()->on_create_request(get_server(), size, self);
	/* Send answer
	 */
	int answer_cmd[3];
	answer_cmd[0] = CREATE_ANSWER;
	answer_cmd[1] = id;
	answer_cmd[2] = -1;
	struct output_chunk* chunk = alloc_chunk_with_data((char*)answer_cmd, sizeof(answer_cmd));
	if (!chunk) die("CREATE_REQUEST: chunk allocation failed");
	add_chunk_to_output_fifo(self, chunk);
	printf("New allocation at %d(size=%d)\n", id, size);
      }
      break;
    case CREATE_ANSWER:
      {
	unsigned id = (unsigned) buf[1];
	get_server()->on_create_answer(get_server(), id, self);
      }
      break;
    default:
...
{
  int alloc_cmd[3];
  alloc_cmd[0] = CREATE_REQUEST;
  alloc_cmd[1] = -1;
  alloc_cmd[2] = size;
  struct output_chunk* chunk = alloc_chunk_with_data((char*)alloc_cmd, sizeof(alloc_cmd));
  if (!chunk) die("host_add_chunk_contents: chunk allocation failed");
  add_chunk_to_output_fifo(self, chunk);  
}
